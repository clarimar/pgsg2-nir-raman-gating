"""
Script de execução do protocolo experimental H1 (pgsg_2, ramo Raman) --
VERSÃO CORRIGIDA, usando PGSGv2Model (sigmoid + MLP + Adam), o modelo
que de fato gerou os resultados publicados de pgsg_1 (results_v2/).

A versão anterior deste script (run_h1_raman.py) usava PGSGModel
(softmax + PLS + diferença finita), uma iteração anterior nunca usada
para os resultados finais do artigo -- ver correção registrada em
memória/ADR. Este script substitui aquele.

pgsg_v2.py NÃO faz parte do pacote instalável pgsg_1 (é um arquivo
solto na raiz do repositório pgsg_1, usado via sys.path por
run_experiment_v2.py). Este script replica exatamente essa forma de
importação, sem modificar a estrutura de pgsg_1.

Uso:
    python scripts/run_h1_raman_v2.py --pgsg1-root /home/clarimar/Dropbox/pgsg/pgsg_1 --quick
    python scripts/run_h1_raman_v2.py --pgsg1-root /home/clarimar/Dropbox/pgsg/pgsg_1
"""

from __future__ import annotations

import argparse
import sys
import time
from pathlib import Path

import numpy as np
from sklearn.metrics import r2_score
from sklearn.model_selection import train_test_split

from pgsg2.ingestion.raman_bioprocess import RamanBioprocessSubstratesLoader
from pgsg2.preprocessing.raman import RamanPreprocessor
from pgsg2.priors.raman import RamanGlucosePrior
from pgsg2.models.adapter import to_pgsg1_dataset
from pgsg2.interpretability.topology import gate_topology

from pgsg_1.ingestion import SpectralDataset as PGSG1SpectralDataset
from pgsg_1.models.pls import PLSModel


def _subset_dataset(ds1: PGSG1SpectralDataset, n_samples: int, n_bands: int, seed: int) -> PGSG1SpectralDataset:
    rng = np.random.default_rng(seed)
    n_samples = min(n_samples, ds1.n_samples)
    sample_idx = rng.choice(ds1.n_samples, size=n_samples, replace=False)
    sample_idx.sort()
    n_bands = min(n_bands, ds1.n_bands)
    band_idx = np.linspace(0, ds1.n_bands - 1, n_bands).round().astype(int)
    band_idx = np.unique(band_idx)
    return PGSG1SpectralDataset(
        X=ds1.X[np.ix_(sample_idx, band_idx)],
        y=ds1.y[sample_idx],
        wavelengths=ds1.wavelengths[band_idx],
        metadata=dict(ds1.metadata),
    )


def _split(ds1: PGSG1SpectralDataset, test_frac: float, seed: int):
    idx = np.arange(ds1.n_samples)
    train_idx, test_idx = train_test_split(idx, test_size=test_frac, random_state=seed)
    train = PGSG1SpectralDataset(X=ds1.X[train_idx], y=ds1.y[train_idx], wavelengths=ds1.wavelengths, metadata=dict(ds1.metadata))
    test = PGSG1SpectralDataset(X=ds1.X[test_idx], y=ds1.y[test_idx], wavelengths=ds1.wavelengths, metadata=dict(ds1.metadata))
    return train, test


def main():
    parser = argparse.ArgumentParser(description="Protocolo H1 (Raman) -- PGSGv2Model (sigmoid+MLP+Adam) vs PLS")
    parser.add_argument("--pgsg1-root", type=str, required=True, help="Caminho raiz do repositório pgsg_1 (onde está pgsg_v2.py)")
    parser.add_argument("--quick", action="store_true")
    parser.add_argument("--seed", type=int, default=42)
    parser.add_argument("--test-frac", type=float, default=0.2)
    parser.add_argument("--hidden", type=int, default=32)
    parser.add_argument("--max-epochs", type=int, default=500)
    parser.add_argument("--patience", type=int, default=30)
    parser.add_argument("--n-components-pls", type=int, default=10)
    args = parser.parse_args()

    sys.path.insert(0, str(Path(args.pgsg1_root).resolve()))
    from pgsg_v2 import PGSGv2Model  # import tardio, depende do sys.path acima

    t_start = time.time()

    def log(msg: str):
        print(f"[{time.time() - t_start:7.1f}s] {msg}", flush=True)

    log("Carregando bioprocess_substrates via raman-data ...")
    ds2 = RamanBioprocessSubstratesLoader().load()
    log(f"Carregado: n={ds2.n_samples}, p={ds2.n_bands}")

    log("Pré-processando (ALS + cosmic rays + SNV) ...")
    ds2 = RamanPreprocessor().fit_transform(ds2)
    log("Pré-processamento concluído")

    ds1 = to_pgsg1_dataset(ds2, target_unit="unknown")

    if args.quick:
        log("Modo --quick: subamostrando para calibração de tempo")
        ds1 = _subset_dataset(ds1, n_samples=300, n_bands=200, seed=args.seed)
        max_epochs, patience = 30, 10
    else:
        max_epochs, patience = args.max_epochs, args.patience

    log(f"Dataset final: n={ds1.n_samples}, p={ds1.n_bands}")

    train, test = _split(ds1, test_frac=args.test_frac, seed=args.seed)
    log(f"Split: treino n={train.n_samples}, teste n={test.n_samples}")

    prior = RamanGlucosePrior().compute(train.wavelengths)
    log("Prior de glicose calculado (bandas vibracionais literárias)")

    log("Treinando PLSModel (baseline) ...")
    t0 = time.time()
    pls = PLSModel(n_components=args.n_components_pls).fit(train)
    log(f"PLSModel treinado em {time.time() - t0:.1f}s")

    log(f"Treinando PGSGv2Model (sigmoid+MLP+Adam; hidden={args.hidden}, max_epochs={max_epochs}, patience={patience}) ...")
    t0 = time.time()
    pgsg = PGSGv2Model(
        hidden=args.hidden, max_epochs=max_epochs, patience=patience, seed=args.seed,
    ).fit(train, prior=prior)
    log(f"PGSGv2Model treinado em {time.time() - t0:.1f}s "
        f"(best_epoch={pgsg.train_history['best_epoch']})")

    r2_pls_test = r2_score(test.y, pls.predict(test))
    r2_pgsg_test = r2_score(test.y, pgsg.predict(test))
    delta_r2_pls = r2_pgsg_test - r2_pls_test

    print()
    print("=" * 60)
    print(f"R2 PLS     (teste): {r2_pls_test:.4f}")
    print(f"R2 PGSGv2  (teste): {r2_pgsg_test:.4f}")
    print(f"Delta_R2_PLS = R2(PGSGv2) - R2(PLS) = {delta_r2_pls:+.4f}")
    print(f"H1 {'SUPORTADA' if delta_r2_pls > 0 else 'NAO suportada'} nesta execução")
    print("=" * 60)

    g = pgsg.gates
    topo = gate_topology(g)  # gate_entropy renormaliza com aviso, pois sigmoid nao soma 1
    rho = float(np.corrcoef(g, prior)[0, 1])
    print(f"rho(gate, prior) = {rho:.6f}")
    print(f"Topologia do gate: entropy={topo['entropy']:.4f}  "
          f"smoothness={topo['smoothness']:.6f}  sparsity_hoyer={topo['sparsity_hoyer']:.4f}")

    np.savez(
        "h1_raman_v2_result.npz",
        gate=g, prior_used=prior, wavelengths=train.wavelengths,
        r2_pls=r2_pls_test, r2_pgsg=r2_pgsg_test,
        best_epoch=pgsg.train_history["best_epoch"],
        train_losses=pgsg.train_history["train_losses"],
        val_losses=pgsg.train_history["val_losses"],
        rho_gate_prior=rho, **topo,
        n_train=train.n_samples, n_test=test.n_samples, n_bands=train.n_bands,
    )
    log("Resultado salvo em h1_raman_v2_result.npz")


if __name__ == "__main__":
    main()
