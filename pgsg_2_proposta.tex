...
Reuso integral do \texttt{PGSGModel} de \texttt{pgsg\_1} (\texttt{src/pgsg\_1/models/pgsg.py}): gate por \emph{softmax} sobre um vetor $\theta \in \mathbb{R}^p$ ($g = \mathrm{softmax}(\theta)$), regressor interno PLS reajustado a cada época sobre $X \odot g$, e gradiente de $\theta$ estimado por diferença finita central (sem autograd). Treino por SGD com momentum e early stopping por \emph{val\_loss}. Nenhuma mudança arquitetural é introduzida entre modalidades --- esta é, precisamente, a condição de teste da Hipótese~1.
...
Seja $g = \mathrm{softmax}(\theta) \in (0,1)^p$ o vetor de gate aprendido pelo \texttt{PGSGModel} sobre $p$ bandas/números de onda. Por construção do softmax, $\sum_{i=1}^p g_i = 1$ e $g_i > 0$ para todo $i$ -- logo a entropia de Shannon se aplica diretamente a $g$, sem normalização adicional.

\begin{align}
\text{Entropia:}\quad & H(g) = -\sum_{i=1}^{p} g_i \log g_i \\
