...
\texttt{pgsg\_1} encerrou-se em estado de submissão (artigo \emph{"Prior-Guided Spectral Gating with End-to-End Optimization for NIR Spectroscopy: Scalable Architecture for Interpretable Regression in Small-Sample Regimes"}, formato \texttt{elsarticle}, periódico-alvo \emph{Chemometrics and Intelligent Laboratory Systems} --- Chemolab). \texttt{pgsg\_2} reaproveita a arquitetura \texttt{PGSGv2Model} (\texttt{pgsg\_v2.py}) e a filosofia de contratos por estágio (operadores $\mathcal{G}, \mathcal{T}, \mathcal{P}, \mathcal{C}, \mathcal{M}, \mathcal{I}_{\text{pred}}, \mathcal{I}_{\text{interp}}, \Sigma$) já implementada e testada em \texttt{pgsg\_1}.

\textbf{Nota de proveniência do código.} O módulo \texttt{pgsg\_v2.py} não faz parte do pacote instalável \texttt{pgsg\_1} (é um arquivo na raiz do repositório, importado via manipulação de \texttt{sys.path} pelo script \texttt{run\_experiment\_v2.py}, o mesmo que gerou os resultados publicados em \texttt{results\_v2/}). Existe também, dentro de \texttt{src/pgsg\_1/models/}, um módulo mais antigo chamado \texttt{PGSGModel} (softmax global + PLS + diferença finita) que foi uma iteração anterior, nunca utilizada para os resultados finais do artigo. \texttt{pgsg\_2} reaproveita exclusivamente \texttt{PGSGv2Model}, replicando a mesma forma de importação usada por \texttt{run\_experiment\_v2.py}, sem modificar a estrutura de \texttt{pgsg\_1}.
...
\item \textbf{Formalização matemática dos descritores de topologia do gate.} As expressões de entropia, suavidade e esparsidade na pré-proposta precisam de correção: o gate real (\texttt{PGSGv2Model}, sigmoid independente por banda) \emph{não} soma 1 --- ao contrário do softmax, cada $g_i$ é independente em $(0,1)$ --- portanto a entropia de Shannon exige uma normalização prévia (Seção~\ref{sec:topologia}); e a razão de esparsidade proposta na pré-proposta não corresponde a nenhuma medida padrão da literatura de processamento de sinais. A Seção~\ref{sec:topologia} apresenta as definições corrigidas.
\end{enumerate}
...
\noindent\textbf{Nota sobre escala.} Diferentemente da pré-proposta original --- que temia um regime de amostra tipicamente pequeno em Raman (mediana de 235 espectros entre os 58 datasets de regressão do RamanBench, 87\% abaixo de 500 amostras) --- o dataset selecionado (\texttt{bioprocess\_substrates}, $n=6{.}472$) está na mesma ordem de grandeza do ramo NIR (Mango DMC v3, $n\approx 12{.}000$), o que permite testar as Hipóteses 1--5 sem o fator de confusão de escalas de amostra muito diferentes entre modalidades.
...
Reuso integral do \texttt{PGSGv2Model} de \texttt{pgsg\_1} (\texttt{pgsg\_v2.py}): gate por \emph{sigmoid independente por banda} sobre um vetor $\theta \in \mathbb{R}^p$ ($g_i = \sigma(\theta_i)$, cada $g_i \in (0,1)$ independentemente, sem soma normalizada a 1), regressor MLP leve (uma camada oculta, 32 unidades, ReLU + dropout) aplicado a $X \odot g$ e treinado \emph{fim-a-fim} via autograd (PyTorch, otimizador Adam, \emph{weight\_decay} $=10^{-4}$). Early stopping por \emph{val\_loss}. Nenhuma mudança arquitetural é introduzida entre modalidades --- esta é, precisamente, a condição de teste da Hipótese~1.

\subsection{Priors por modalidade}
\begin{itemize}
\item \textbf{NIR:} prior de literatura fixo (\texttt{make\_literature\_prior}, três patamares por região espectral: 680--700~nm $=0{,}6$ --- clorofila; 900--1000~nm $=1{,}0$ --- 2º sobretom C-H; 1100--1200~nm $=0{,}8$ --- 1º sobretom C-H; $0{,}1$ nas demais bandas), independente dos dados de treino --- condição estabelecida em \texttt{pgsg\_1} para evitar circularidade gate-prior.
\item \textbf{Raman:} prior construído a partir de atribuições vibracionais conhecidas da literatura para o(s) analito(s)-alvo do dataset selecionado (bandas de glicose 911/1060/1125~cm$^{-1}$, alta confiança; conjunto secundário de bandas de meio de cultura, confiança menor --- Seção de priors do repositório), nunca derivado dos dados de treino do próprio experimento. Implementado como soma de gaussianas centradas nas bandas literárias (em vez de patamares constantes como no NIR), por ser mais informativo sobre a largura de linha esperada --- diferença de estilo de codificação do prior, não de princípio anti-circularidade.
\end{itemize}

\subsection{Protocolo experimental}
Protocolo intra-domínio (\emph{within-dataset}), replicando a decisão de \texttt{pgsg\_1} de evitar comparações entre domínios heterogêneos sem validação prévia (cf.\ degradação observada no protocolo cross-safra, $R^2_{\text{PLS}}$ caindo para $\approx 0{,}06$). Conjunto de teste fixo, disjunto e de tamanho constante por dataset.

\subsection{Topologia do gate espectral}
\label{sec:topologia}

Seja $g \in (0,1)^p$ o vetor de gate aprendido pelo \texttt{PGSGv2Model} sobre $p$ bandas/números de onda, com $g_i = \sigma(\theta_i)$ independente por banda. Diferentemente de um gate softmax, $\sum_i g_i \neq 1$ em geral --- portanto a entropia de Shannon exige normalização prévia:

\begin{align}
\text{Entropia:}\quad & H(g) = -\sum_{i=1}^{p} \hat{g}_i \log \hat{g}_i, \qquad \hat{g}_i = \frac{g_i}{\sum_{j=1}^p g_j} \\
...
$S(g)=0$ corresponde ao gate uniformemente denso; $S(g)=1$ corresponde a um gate \emph{one-hot}. Esta formalização substitui a razão $\|g\|_2/\|g\|_1$ da pré-proposta original, que não é uma medida de esparsidade padrão nem tem direção de interpretação definida. (Nota de implementação: a normalização $\hat g$ é aplicada automaticamente, com aviso, sempre que $\sum_i g_i \neq 1$ --- caso do gate sigmoid real, ao contrário do que uma versão anterior deste documento afirmava.)
...
\item \textbf{Proveniência do código de \texttt{pgsg\_1}:} o repositório contém mais de uma implementação de PGSG (\texttt{PGSGModel} em \texttt{src/}, \texttt{PGSGv2Model} em \texttt{pgsg\_v2.py}, \texttt{PGSGModelTorch} em \texttt{pgsg\_torch.py}). Apenas \texttt{PGSGv2Model} corresponde aos resultados publicados (\texttt{results\_v2/}); qualquer reuso futuro de código de \texttt{pgsg\_1} deve confirmar contra o script de experimento real (\texttt{run\_experiment\_v2.py}), não apenas contra o histórico de commits de um arquivo isolado.
\end{itemize}
...
Meses & Atividade \\
\midrule
1--2 & Escopo de dados: inspeção do \texttt{raman-data} e seleção do dataset Raman \textbf{(concluído --- ADR-0001, \texttt{bioprocess\_substrates})}; extensão do contrato $\mathcal{G}$ (ingestão) para múltiplas modalidades \textbf{(concluído)}. \\
3--4 & Extensão do operador $\mathcal{T}$ (pré-processamento específico de Raman: ALS, remoção de \emph{cosmic rays}); testes de contrato \textbf{(concluído)}. \\
5--6 & Definição do prior fisicamente motivado para Raman (operador $\mathcal{P}$) \textbf{(concluído)}; execução do protocolo experimental intra-domínio para NIR e Raman com \texttt{PGSGv2Model} sem alteração arquitetural (operadores $\mathcal{C}, \mathcal{M}, \mathcal{I}_{\text{pred}}$) \textbf{--- Raman concluído (H1 suportada, $\Delta R^2_{\text{PLS}}=+0{,}0246$); NIR em andamento}. \\
7--8 & Implementação e cálculo dos descritores de topologia do gate (entropia, suavidade, esparsidade) no operador $\mathcal{I}_{\text{interp}}$ \textbf{(concluído)}; testes estatísticos das Hipóteses 2--5 (pendente --- depende do gate NIR). \\
...
$\mathcal{P}$ & \texttt{priors} & Novo tipo de prior: atribuição vibracional literatura-informada (gaussianas, não patamares constantes como em NIR), com mesmo princípio anti-circularidade. \\
...
$\mathcal{M}$ & \texttt{models} & Reuso integral de \texttt{PGSGv2Model}, \texttt{PLSModel}; nenhuma mudança arquitetural entre modalidades (condição da Hipótese 1). Importação via \texttt{sys.path} de \texttt{pgsg\_v2.py}, replicando \texttt{run\_experiment\_v2.py}. \\
...
\item Saranwong, S., Sornsrivichai, J., Kawano, S. \emph{Prediction of ripe-stage eating quality of mango fruit from its harvest quality measured nondestructively by near infrared spectroscopy}. Postharvest Biology and Technology, 31, 253--261, 2004.
\item Guthrie, J. et al. \emph{Robustness across season, cultivar for NIR calibration of mango dry matter}. Australian Journal of Agricultural Research, 56, 1197--1204, 2005.
\end{itemize}
